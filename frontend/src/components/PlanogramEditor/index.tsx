...
export default function PlanogramEditor({ projectId, planogramId, onClose }: PlanogramEditorProps) {
  ...
  const cellW = Math.max(CELL_MIN_PX, Math.min(CELL_MAX_PX * 3, Math.round(physCellW * CELL_WIDTH_SCALE  * zoom)));
  const cellH = Math.max(CELL_MIN_PX, Math.min(CELL_MAX_PX * 3, Math.round(physCellH * CELL_HEIGHT_SCALE * zoom)));

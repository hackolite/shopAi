...
_FORWARDED_SESSION_PREFIXES = ("/api/cad/projects", "/api/furniture-library")
...
def _request_uses_forwarded_session(request: Request) -> bool:
    return SESSION_HEADER_NAME in request.headers and any(
        request.url.path == prefix or request.url.path.startswith(prefix + "/")
        for prefix in _FORWARDED_SESSION_PREFIXES
    )
...
def _normalize_store_layout_payload(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    try:
        ...
        planograms = [
            project_manager._project_validated_shape(
